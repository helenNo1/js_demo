...
//    wx.cloud.init();
  ...
  async onTapSendButton() {
    ...
    this.setData({ showStartPage: false, prompt: "" });
		...
		const { send, handleMessage, handleClose, handleError} = await useGPT(history);
    ...
    handleMessage((piece: string) => {
			console.log(piece)
			conversation.appendAssistantMessage(piece);
			final();
		});
    send();
  },
